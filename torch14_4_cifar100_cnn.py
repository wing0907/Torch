...
path = './_data/torch/'
train_dataset = CIFAR100(path, train=True, download=True, transform=transf) #.to(DEVICE)
test_dataset = CIFAR100(path, train=False, download=True, transform=transf)  #.to(DEVICE)
...
class CNN(nn.Module):
    def __init__(self, in_channels, num_classes=100):
        ...
        # after 3×(32→30→15→13→6→4→2) pools you end up at (16,2,2):
...
# instantiate for CIFAR-100
model = CNN(in_channels=3, num_classes=100).to(DEVICE)  # torch에서는 channel 만 input으로 넣어줌. 나머지는 알아서 맞춰줌
...
# Final Loss: 4.08503759783297
# Final ACC: 0.07008786