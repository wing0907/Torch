...
x = torch.FloatTensor(x).unsqueeze(1)       # unsqueeze 하면 차원을 늘린다. () 안은 순서. 0 하면 ( , 100, 10) 제일 앞쪽이 1이 됨 // 현재 1이기 때문에 (3, 1)이 됨
print(x)
# tensor([[1.],
#         [2.],
#         [3.]])
...
model = nn.Linear(1, 1)             # 하지만 여기서는 앞이 input. 뒤에 output    # y = xw + b
...
result = model(torch.Tensor([[4]])) # 2차원이니깐 요로케 넣어준다
