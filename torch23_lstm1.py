...
class LSTM(nn.Module):
    def __init__(self):
        ...
        # self.rnn_layer2 = nn.LSTM(32, 32, batch_first=True) # 위에 주석하고 이렇게 처리해도 돌아감.
        self.fc1 = nn.Linear(32, 16)  # fully connected layer = 전체가 전부 연결된 구조. Dense 밀집 
    ...
    def forward(self, x, h0, c0):
        if h0 == None:
            h0 = torch.zeros(1, x.size(0), 32).to(DEVICE) # hidden_state의 초기값이다
        if c0 == None:    
            c0 = torch.zeros(1, x.size(0), 32).to(DEVICE) # hidden_state의 초기값이다
            #  num_layers, batch_size, hidden_size(output node의 개수)
        x, (hn, cn) = self.lstm_layer1(x, (h0, c0))   
        
    # def forward(self, x):    
        # x, _ = self.lstm_layer1(x) # 이게 위에 3줄임
