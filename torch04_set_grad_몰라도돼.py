...
x = torch.FloatTensor(x).to(DEVICE)       # unsqueeze 하면 차원을 늘린다. () 안은 순서. 0 하면 ( , 100, 10) 제일 앞쪽이 1이 됨 // 현재 1이기 때문에 (3, 1)이 됨
print(x)
...
# model.parameters() = 모델을 전부 SGD로 가중치 갱신을 하겠다 // 모든 parameters.
...
print('10, 1.3의 예측값 :', result)           # 4의 예측값 : tensor([[4.]], grad_fn=<AddmmBackward0>)
print('10, 1.3의 예측값 :', result.item())    # 4의 예측값 : 4.0
